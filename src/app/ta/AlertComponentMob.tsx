...
const AlertsComponentMob: React.FC<AlertsComponentProps> = ({ userId }) => {
  const [alerts, setAlerts] = useState<Alert[]>([]);

  useEffect(() => {
    const alertsRef = ref(database, `users/${userId}/alert-ta`);
    ...
    <div className="w-[300px] h-[180px] relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col gap-[13px] pt-[37px] px-[20px]">
        {alerts.slice(0, 4).map((alert, index) => (
          <div
            key={alert.id}
            className="w-[280px] h-[61px] rounded-[30px] flex items-center px-[26px] gap-[39px] transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]"
            style={{
...
export default AlertsComponentMob;